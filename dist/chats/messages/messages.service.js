...
let MessagesService = class MessagesService {
    constructor(chatReponsitory, usersService, pubSub) {
        this.chatReponsitory = chatReponsitory;
    ...
    async createMessage({ content, chatId }, userId) {
        ...
        await this.chatReponsitory.findOneAndUpdate({
            _id: chatId,
        }, {
            $push: {
    ...
    async getMessages({ chatId, skip, limit }) {
        const messages = await this.chatReponsitory.model.aggregate([
    ...
    async countMessages(chatId) {
        return (await this.chatReponsitory.model.aggregate([
            { $match: { _id: new mongoose_1.Types.ObjectId(chatId) } },
            { $unwind: '$messages' },
            { $count: 'messages' },
        ]))[0];
